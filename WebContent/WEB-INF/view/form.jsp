			...
			<form action="" method="post" enctype="multipart/form-data">
				<p class="mb-0">
					...
					<div class="preview-img mb-2">
						<img class="w-75" src="<c:if test="${!empty image}"><%= request.getContextPath() %>/images/uploads/<c:out value="${image}" /></c:if>">
					</div>
				...
				<p>
					...
					<input type="text" name="name" size="40" class="mt-1" <c:if test="${!empty name}"> value="<c:out value="${name}" />"</c:if>>
				...
				<p>
					...
					<input type="text" name="kana" size="40" class="mt-1" <c:if test="${!empty kana}"> value="<c:out value="${kana}" />"</c:if>>
				...
				<p>
					<div class="d-inline-block w-40">調味料<span class="form-require ml-2 text-light px-2 bg-danger rounded">必須</span></div>
					<div class="d-inline-block w-40">分量<span class="form-require ml-2 text-light px-2 bg-danger rounded">必須</span></div>
					<c:forEach var="j" begin="0" end="${menuFoodLength - 1}">
						<div class="menu-food mb-2">
							<select name="food_id" class="select-foods w-40 mt-1">
								<c:forEach var="food" items="${foods}">
									<option value="<c:out value="${food.id}" />" <c:if test="${food.id == foodIds[j]}">selected</c:if>><c:out value="${food.name}" /></option>
								</c:forEach>
							</select>
							<input type="number" name="food_quantity" class="select-numbers text-right mt-1 w-30"
				...
				<p>
					食材<br>
					<textarea name="foodstuff" cols="45" rows="8"><c:if test="${foodstuff != null}"><c:out value="${foodstuff}" /></c:if></textarea>
				</p>
				<p>
					レシピ<br>
					<textarea name="recipe" cols="45" rows="8"><c:if test="${recipe != null}"><c:out value="${recipe}" /></c:if></textarea>
				</p>
				<p class="mb-0">
					<c:if test="${empty url}">
